...
@NamedQueries({
        ...
        @NamedQuery(name = Meal.GET_BETWEEN_HALF_OPEN, query =
                "SELECT m FROM Meal m WHERE m.user.id=:user_id AND m.dateTime >=:startDateTime AND m.dateTime <:endDateTime " +
        ...
        @NamedQuery(name = Meal.UPDATE, query =
                "UPDATE Meal m SET m.description=:description, m.dateTime=:dateTime, m.calories=:calories " +
                        "WHERE m.id=:id AND m.user.id=:user_id"),

        @NamedQuery(name = Meal.GET, query = "SELECT m FROM Meal m WHERE m.id=:id AND m.user.id=:user_id ORDER BY m.dateTime DESC")
})
@Entity
@Table(name = "meals" ,
